          ...
          >
            {drop.chars.map((char, charIndex) => {
              ...
              return (
                <div
                  key={charIndex}
                  className={`text-lg ${isLastChar ? "text-red-300" : "text-red-500"}`}
                  style={{
                    opacity: Math.min(1, charOpacity), // Ensure opacity doesn't exceed 1
                    textShadow: isLastChar
                      ? `0 0 ${8 * drop.glowIntensity}pxrgb(253, 71, 80)`
                      : isNearLastChar
                        ? `0 0 ${3 * drop.glowIntensity}pxrgb(199, 2, 51)`
                        : "none",
                  }}
                >
                  {char}
